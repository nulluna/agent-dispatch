...
describe('routing', () => {
  ...
  it('creates transparent routes with https defaults', () => {
    expect(
      createTransparentRoute(
        new URL('https://dispatch.local/api/user/self?from=browser'),
        'api.example.com',
      ),
    ).toEqual({
      kind: 'proxy',
      ...
      targetPathname: '/api/user/self',
      targetSearch: '?from=browser',
    })
  })
})
