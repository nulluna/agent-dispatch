...
export function parseDispatchRoute(requestUrl: URL): DispatchRoute {
  ...
  const protocolCode = protocolCodeRaw as ProtocolCode
  const targetHost = validateHost(decodeHost(targetHostRaw))
  const targetPathname = getRouteRemainder(requestUrl.pathname, protocolCode, targetHostRaw)
